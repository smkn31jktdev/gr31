...
import type { BermasyarakatPdfRow, PdfParams } from "../types";

interface BermasyarakatPdfParams extends PdfParams {
  rows: BermasyarakatPdfRow[];
}

export async function downloadBermasyarakatStyledPdf(
  params: BermasyarakatPdfParams,
): Promise<void> {
  ...
  const columnWidths = [38, 60, 40, 32];
  const periodLabel =
  ...
  const drawHeader = (startY: number): number => {
    ...
    x += columnWidths[0];
    doc.text("Jenis Kegiatan", x + 2, startY + 6.5);
    x += columnWidths[1];
    doc.text("Tempat", x + 2, startY + 6.5);
    x += columnWidths[2];
