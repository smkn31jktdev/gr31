...
## API Testing (MongoDB/AstraDB)

Backend HTTP API dapat dijalankan tanpa PostgreSQL untuk kebutuhan testing endpoint utama (auth admin/siswa, kehadiran, kegiatan, aduan, dan manajemen admin/siswa).

Panduan Postman lengkap tersedia di:

- [Postman API Testing Guide](./design-docs/postman-api-testing.md)
...
Make sure required external dependencies are running based on your selected drivers. For example, if using `none` for message/cache/workflow drivers, RabbitMQ/Redis/Temporal are not required.
