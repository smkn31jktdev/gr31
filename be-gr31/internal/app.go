...
import (
	...
	mongodb_outbound_adapter "prabogo/internal/adapter/outbound/mongodb"
	noop_outbound_adapter "prabogo/internal/adapter/outbound/noop"
...
var databaseDriverList = []string{"mongodb"}
var httpDriverList = []string{"fiber"}
var messageDriverList = []string{"rabbitmq", "none", ""}
...
func databaseOutbound(ctx context.Context) outbound_port.DatabasePort {
	...
	case "mongodb":
		return mongodb_outbound_adapter.NewAdapter()
	}
	return nil
}
...
func messageOutbound(ctx context.Context) outbound_port.MessagePort {
	...
	if outboundMessageDriver == "" || outboundMessageDriver == "none" {
		return noop_outbound_adapter.NewMessageAdapter()
	}
...
func cacheOutbound(ctx context.Context) outbound_port.CachePort {
	if !utils.IsInList([]string{"redis", "none", ""}, outboundCacheDriver) {
		log.WithContext(ctx).Fatal("cache driver is not supported")
		os.Exit(1)
	}

	if outboundCacheDriver == "" || outboundCacheDriver == "none" {
		return noop_outbound_adapter.NewCacheAdapter()
	}
...
func workflowOutbound(ctx context.Context) outbound_port.WorkflowPort {
	...
	if outboundWorkflowDriver == "" || outboundWorkflowDriver == "none" {
		return noop_outbound_adapter.NewWorkflowAdapter()
	}
